...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/css/null1.css">
...
<div class="links">
    ...
    <?php
            ...
            while($boards = pg_fetch_array($query, null, PGSQL_ASSOC))
                echo " <a href='/".$boards["uri"]."'>".$boards["uri"]."</a> |";
        ?>
</div>
...
<header>
    ...
    <div class="welcome">
        <img src="/img/<?php echo $board["banner_uri"] ?>" width="375" height="215" >
        <h1><?php echo "/<a href='/".$board["uri"]."'>".$board["uri"]."</a>/ - ".$board["name"] ?></h1>
        <h2><?php echo $board["description"] ?></h2>
    </div>
    <hr>
    <div class="form-post">
        </p><a href="#" id="create-thread" onclick="postFormDisplay('<?php echo $status ?>');"><p>Создать тред</p></a>
