<?php
require_once("metrix.html");
//Never expiring date for cookies
const NEVEREXPIRING = 10 * 365 * 24 * 60 * 60;  //10 years
const UPLOADDIR = '/var/www/u121092/data/www/13ch.tk/img/upload/';
const MAXFILESIZE = 5 * 1024 * 1024; //5MB
const SHORTTEXT = 666;
//Database Connection
$dbconn = pg_connect("host=localhost dbname=u121092_chan user=u121092_chan password=ldw;qodk2");
//Board metainfo
...
function getAllPosts($thread_id) {
    getSafeID($thread_id);
    $query = pg_query("SELECT * FROM posts WHERE thread_id=$thread_id");
...
function getAllThreads($board_uri) {
    ...
    return pg_query("SELECT thread_id FROM posts WHERE id in (SELECT DISTINCT ON (thread_id) id FROM 
                    posts 
...
function sendPost($thread_id) {
    ...
    if(isset($_FILES['file']['name'])) {
        ...
        for($i = 0; $i<$total_files; $i++) {
            ...
            $uploadfile = UPLOADDIR.$thread_id.'/'.basename($_FILES['file']['name'][$i]);
            $file = $thread_id.'/'.basename($_FILES['file']['name'][$i]);
...
function createThread($board_id) {
    ...
    sendPost($id);
    header("Location: null.php?board=".$_GET['board']."&&thread=".$id);
}
        ...
        <article class="<?php if($main_post) echo "main-post"; else echo "post"; $main_post=false; ?>">
            ...
            <div class="pictures">
                ...
                while($pictures=pg_fetch_array($request)) {
                    ...
                    <a class="picture" href="/img/upload/<?php echo $pictures["uri"] ?>">
                        <p><?php echo shortFileURI($pictures["uri"]) ?></p>
                        <img width="100" height="100" src="/img/upload/<?php echo $pictures["uri"] ?>"/>
                    </a>
                    <?php
                }
                ?>
